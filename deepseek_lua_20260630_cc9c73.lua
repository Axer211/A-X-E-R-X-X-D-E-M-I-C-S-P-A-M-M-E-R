...
-- ============================================
-- [[ MAIN FRAME – wider & shorter ]]
-- ============================================
local frame = Instance.new("Frame", ScreenGui)
frame.Size = UDim2.new(0, 380, 0, 330)   -- width 380, height 330
...
-- [[ OPEN / CLOSE & MINIMIZE functions (defined early) ]]
...
local function ToggleMinimize()
    if minimized then
        ...
        -- Show all content except the title bar? Actually we need to show everything.
        -- We'll just make the Pages and TabBar visible again.
        TabBar.Visible = true
        Pages.Visible = true
        -- Also ensure the minimize button text changes
        minBtn.Text = "🗕"
    else
        -- Minimize: shrink to just the title bar
        minimized = true
        TweenService:Create(frame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
-- Minimize button (on the right side of title bar)
...
Pages.Position = UDim2.new(0, 0, 0, 58)
...
local function MakeBtn(parent, text, yPos, bgColor, h)
    ...
    b.Size = UDim2.new(0.9, 0, 0, h or 32)
    b.Position = UDim2.new(0.05, 0, 0, yPos)
...
local function MakeToggle(parent, text, yPos, callback, defaultValue)
    ...
    lbl.Text = text; lbl.Size = UDim2.new(0.6, 0, 0, 26)
    ...
    tog.Text = "OFF"; tog.Size = UDim2.new(0, 46, 0, 20)
    tog.Position = UDim2.new(1, -54, 0, yPos + 3)
...
-- ============================================
-- [[ ATTACK PAGE (adjusted for shorter height) ]]
-- ============================================
local AttackScroll = Instance.new("ScrollingFrame", AttackPage)
AttackScroll.Size = UDim2.new(1, 0, 1, 0)
AttackScroll.BackgroundTransparency = 1
AttackScroll.ScrollBarThickness = 2
AttackScroll.CanvasSize = UDim2.new(0, 0, 0, 350) -- reduced to fit

local nameBox  = MakeBox(AttackScroll, "Target Name...", 6)
local delayBox = MakeBox(AttackScroll, "Delay seconds (default: 2)", 40)

local attackStart = MakeBtn(AttackScroll, "START ATTACK 🔥", 76, Color3.fromRGB(135, 206, 235), 32)
local attackStop  = MakeBtn(AttackScroll, "STOP ATTACK",     116, Color3.fromRGB(200, 80, 80), 32)

local customModeToggle = MakeToggle(AttackScroll, "CUSTOM SPAM MODE", 156, function(v) 
...
local customMessageBox = MakeBox(AttackScroll, "Enter custom message...", 192)
customMessageBox.Visible = false

local addCustomBtn = MakeBtn(AttackScroll, "➕ ADD MESSAGE", 230, Color3.fromRGB(100, 150, 220), 26)
addCustomBtn.Visible = false

local customMessageList = Instance.new("TextLabel", AttackScroll)
customMessageList.Size = UDim2.new(0.9, 0, 0, 40)
customMessageList.Position = UDim2.new(0.05, 0, 0, 264)
...
local clearCustomBtn = MakeBtn(AttackScroll, "🗑️ CLEAR ALL MESSAGES", 314, Color3.fromRGB(200, 120, 80), 26)
clearCustomBtn.Visible = false

-- Footer at bottom
local footerLabel = Instance.new("TextLabel", AttackScroll)
footerLabel.Size = UDim2.new(1, 0, 0, 28)
footerLabel.Position = UDim2.new(0, 0, 0, 350) -- bottom
...
-- Update custom message display (same as before)
...
SettingScroll.ScrollBarThickness = 2; SettingScroll.CanvasSize = UDim2.new(0,0,0,320)
...
CreateSettingRow("ANTI-AFK",    4,  function(v) antiAfkEnabled = v end)
CreateSettingRow("SPEED BOOST", 34,  function(v) speedEnabled   = v end)
CreateSettingRow("JUMP BOOST",  64, function(v) jumpEnabled    = v end)
CreateSettingRow("NOCLIP",      94, function(v) noclipEnabled  = v end)
CreateSettingRow("NOSIT",       124, function(v) nositEnabled   = v end)
CreateSettingRow("🛡️ ANTI-COPY (BH)", 154, function(v) 
    antiCopyEnabled = v
    if v then setupAntiCopy() end
end)
CreateSettingRow("🌀 ANTI-FLING", 184, function(v) 
    antiFlingEnabled = v
    if v then setupAntiFling() end
end)
CreateSettingRow("🚫 ANTI-TAG (always ON)", 214, function(v) end)
...
-- ============================================
-- [[ THEME PAGE with smooth transitions ]]
-- ============================================
...
-- ============================================
-- [[ RGB ENGINE for floating button ]]
-- ============================================
task.spawn(function()
    local h = 0
    while true do
