...
-- [[ SLANGS (default spam list) – all TMKX MEH → TMKX MAI ]]
local slangs = {
...
-- [[ ANTI-TAG SPAM SYSTEM (always uses SPAM mode with pattern) ]]
local PATTERN_CYCLE = {
    "`", "/=-=", "Z_", "Q_", "@", "#", "*", "%", "@", "/-", "", "P_",
}
...
-- ============================================
-- [[ ATTACK PAGE (no mode selector, bottom filled with branding) ]]
-- ============================================
...
AttackScroll.CanvasSize = UDim2.new(0, 0, 0, 400) -- enough to hold all items + footer

local nameBox  = MakeBox(AttackScroll, "Target Name...", 8)
local delayBox = MakeBox(AttackScroll, "Delay seconds (default: 2)", 46)

-- Start / Stop buttons
local attackStart = MakeBtn(AttackScroll, "START ATTACK 🔥", 88, Color3.fromRGB(135, 206, 235), 36)
local attackStop  = MakeBtn(AttackScroll, "STOP ATTACK",     132, Color3.fromRGB(200, 80, 80), 36)

-- Custom Spam Mode toggle
local customModeToggle = MakeToggle(AttackScroll, "CUSTOM SPAM MODE", 178, function(v) 
...
local customMessageBox = MakeBox(AttackScroll, "Enter custom message...", 216)
customMessageBox.Visible = false

local addCustomBtn = MakeBtn(AttackScroll, "➕ ADD MESSAGE", 258, Color3.fromRGB(100, 150, 220), 28)
addCustomBtn.Visible = false

local customMessageList = Instance.new("TextLabel", AttackScroll)
customMessageList.Size = UDim2.new(0.9, 0, 0, 44)
customMessageList.Position = UDim2.new(0.05, 0, 0, 296)
...
local clearCustomBtn = MakeBtn(AttackScroll, "🗑️ CLEAR ALL MESSAGES", 350, Color3.fromRGB(200, 120, 80), 28)
clearCustomBtn.Visible = false

-- Branding footer at the bottom of the Attack page
local footerLabel = Instance.new("TextLabel", AttackScroll)
footerLabel.Size = UDim2.new(1, 0, 0, 30)
footerLabel.Position = UDim2.new(0, 0, 0, 390) -- placed at bottom
...
-- [[ OPEN / CLOSE FUNCTIONS (must be defined before use) ]]
...
-- ============================================
-- [[ ATTACK LOGIC (always uses SPAM mode) ]]
-- ============================================
